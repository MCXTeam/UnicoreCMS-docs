...
<Callout type="error" title="Не ставьте true">
`TRUST_PROXY=true` доверяет заголовку `X-Forwarded-For` от кого угодно, то есть любой клиент объявит себя любым адресом и обойдёт все ограничения по IP. На продакшене сервер с таким значением не стартует, в разработке ругается в лог. Указывайте число прокси перед приложением — для встроенного профиля `nginx` это `1`.
</Callout>

<Callout type="warn" title="Пустое значение за прокси — тоже плохо">
Подделать IP так нельзя, но все клиенты схлопнутся в один адрес прокси. Счётчик неудачных входов начнёт считать всех вместе, и первый же перебор заблокирует вход всему сайту разом.
</Callout>

Свой обратный прокси настраивайте так же, как это делают шаблоны из `docker/nginx`: `proxy_set_header X-Forwarded-For $remote_addr;`. Именно `$remote_addr`, а не `$proxy_add_x_forwarded_for` — иначе в цепочку попадёт то, что клиент прислал сам, и подсчёт хопов вернёт подставной адрес.
